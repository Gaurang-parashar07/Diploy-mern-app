...
const Login = ({ setIsAuthenticated }) => {
  ...
  const handleSubmit =  async (e) => {
    ...
    try {
        ...
        const { message: responseMessage, success,jwtToken,name } = result

        if (success) {
          setMessage('Login successful!')
          localStorage.setItem('jwtToken', jwtToken)
          localStorage.setItem("loggedInUser", name)
          // store structured currentUser expected by Dashboard
          try {
            const userObj = { email: form.email, name: name || form.email.split('@')[0] }
            localStorage.setItem('currentUser', JSON.stringify(userObj))
          } catch (e) {
            localStorage.setItem('currentUser', JSON.stringify({ email: form.email, name: name || form.email.split('@')[0] }))
          }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden bg-insta-bg">
      ...
      <div className="w-full max-w-[400px] p-8 glass-panel rounded-2xl shadow-2xl relative z-10 animate-fade-in-up">
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-insta-muted mt-2 font-medium">Log in to your account.</p>
        </div>
      ...
      <div className="w-full max-w-[400px] p-5 glass-panel rounded-2xl mt-4 relative z-10 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
        <p className="text-xs font-medium text-insta-muted">
          ...
          <Link to="/signup" className="text-insta-blue font-bold hover:underline">
            Sign up
          </Link>
        </p>
      </div>
